...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.create_table(
            Table::create()
                ...
                .col(timestamp_with_time_zone(Knowledge::CreatedAt).default(Expr::current_timestamp()))
                .col(timestamp_with_time_zone(Knowledge::UpdatedAt).default(Expr::current_timestamp()))
                .foreign_key(
                    ForeignKey::create()
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.drop_table(Table::drop().table(Knowledge::Table).to_owned()).await
    }
}
