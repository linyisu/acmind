...
async function fetchSolutionData(runId) {
  const resp = await fetch(`${VJUDGE_ORIGIN}/solution/data/${runId}?inPage=true`, {
    method: "POST",
    credentials: "include",
    headers: {
  ...
  if (!resp.ok) throw new Error(`HTTP ${resp.status} for solution/${runId}`);
  return resp.json();
}
...
function extractTitle(html, sourceId) {
  ...
  return decodeEntities(m[1]).split(` - ${sourceId}`)[0].trim() || sourceId;
}
...
function extractUsernameFromPage() {
  ...
  const header = document.querySelector("h1, h2, #header-username, .profile-username");
  if (header) {
...
async function fetchCurrentUsername() {
  ...
  try {
    ...
    if (m1) return m1[1];
    const m2 = html.match(/href="\/user\/([^"]+)"/);
    if (m2) return m2[1];
  } catch {}

  try {
    const resp = await fetchJSON(
      `${VJUDGE_ORIGIN}/status/data?draw=1&start=0&length=1&un=&OJId=All&probNum=&res=all&language=&onlyFollowee=false&orderBy=run_id`,
    );
    if (resp.data?.[0]) return resp.data[0].user ?? resp.data[0].username ?? null;
  } catch {}
  return null;
}
...
async function listSubmissions({ username, ojId, probNum, progress, range }) {
  ...
  for (let page = 0; page < MAX_STATUS_PAGES; page++) {
    ...
    const parsed = resp.data.map(parseSubmission)
      .filter((it) => it.oj === ojId && it.probNum === probNum);
    items.push(...parsed);
...
async function attachSourceCode(submissions, progress, range = [PCT_AFTER_LISTING, PCT_AFTER_SOURCES]) {
  const [pctStart, pctEnd] = range;
  ...
  for (let i = 0; i < submissions.length; i++) {
    ...
    try {
      const data = await fetchSolutionData(submissions[i].runId);
      if (data.code) submissions[i].code = data.code;
    } catch {
      // Source code can be private; ignore and continue.
    }
  }
  progress({ pct: pctEnd });
}
...
async function scrapeProblemFull({ progress }) {
  ...
  if (!username) throw new Error("无法确定您的 VJudge 用户名，请确认已登录");
  ...
  await attachSourceCode(submissions, progress);
