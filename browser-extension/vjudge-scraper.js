...
(function () {
  ...
  function extractPageJson(html, marker) {
    ...
    try {
      return JSON.parse(raw.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/&quot;/g, '"').replace(/&#39;/g, "'"));
    } catch {
      return null;
    }
  }
  ...
  async function scrapeSolutionPage() {
    ...
    if (!data.code && data.codeAccessInfo) {
      throw new Error(`Source code not accessible: ${data.codeAccessInfo.i18nKey || "unknown"}`);
    }
  ...
  async function scrapeProblemSubmissions(oj, probNum, username) {
    ...
    for (let page = 0; page < 200; page++) {
      ...
      const parsed = resp.data.map(parseSubmissionItem)
